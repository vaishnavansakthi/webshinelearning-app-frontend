      ...
      <div className="p-10 max-[600px]:p-5">
        <div className="p-8 max-[600px]:p-1 max-[600px]:mt-12 bg-white dark:bg-[#404040] shadow mt-5">
          ...
          <div className="mt-20 text-center border-b pb-12 max-[600px]:mt-28">
            ...
            <p className="font-light dark:text-[#ffffff] text-gray-600 mt-3">{userData?.[0]?.email}</p>
            <p className="mt-8 dark:text-[#ffffff] text-gray-500">{userData?.[0]?.mobileNumber}</p>
            <p className="mt-2 dark:text-[#ffffff] text-gray-500">{profileData?.users?.[0]?.university}</p>
            
            <p className="mt-2 dark:text-[#ffffff] text-gray-500">{profileData?.users?.[0]?.degree}</p>
            <p className="mt-2 dark:text-[#ffffff] text-gray-500">{profileData?.users?.[0]?.fieldOfStudy}</p>
            <p className="mt-2 dark:text-[#ffffff] text-gray-500">{profileData?.users?.[0]?.graduationYear}</p>
            <p className="mt-2 dark:text-[#ffffff] text-gray-500">{profileData?.users?.[0]?.dateOfBirth}</p>
            ...
            {() => (
              <Form className="mt-3">
                  ...
                  {profileFormSchema.map((form, index) => (
                    <div key={index}>
                      ...
                      <Field
                        ...
                        className="appearance-none mt-1 max-[650px]:mt-2 block w-[220px] max-[650px]:w-[300px] px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                      />
                ...
                <div className="flex justify-end items-end mt-5">
                  ...
                  {/* Ensure that onClick event is correctly bound */}
                  <button
                    type="button"
